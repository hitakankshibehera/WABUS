...
function syncBookedSeatsIntoTrips(tripsList: Trip[]): Trip[] {
  ...
  return tripsList.map(trip => {
    let currentSeats = trip.seats;
    try {
      const customSeatsRaw = localStorage.getItem(`wabus_custom_trip_seats_${trip.id}`);
      if (customSeatsRaw) {
        currentSeats = JSON.parse(customSeatsRaw);
      }
    } catch {}
    ...
    const updatedSeats = (currentSeats || []).map(seat => {
      const seatNumUpper = String(seat.number || '').toUpperCase();
      const seatIdUpper = String(seat.id || '').toUpperCase();
...
export const api = {
  ...
  async updateTripSeats(tripId: string, seats: Seat[]): Promise<{ success: boolean; trip: Trip }> {
    try {
      localStorage.setItem(`wabus_custom_trip_seats_${tripId}`, JSON.stringify(seats));
    } catch {}
